        ...
        <div className='w-full container mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 md:h-20 h-16'>
          {/* logo */}
            <div className='flex items-center gap-1 cursor-pointer'>
              ...
              <div className='w-4 h-4 bg-red-600 -ml-2 rounded-full opacity-75 hover:opacity-100 transition-opacity'></div>
            </div>
          ...
          isMenuOpen && (
            <div className='md:hidden bg-white border-t border-gray-100 py-4'>
              <div className='container mx-auto px-4 space-y-4'>
                {navLink.map((link,index)=>(
                  <a onClick={()=>{setActiveLink(activeLink);setIsMenuOpen(false);}} className={`block font-medium py-2 ${activeLink === link.href ? 'text-blue-600' : 'text-gray-600 hover:text-gray-900'}`} href={link.href}>{link.label}</a>
                ))}
